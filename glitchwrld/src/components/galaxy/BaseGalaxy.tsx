...
export function BaseGalaxy() {
  const pointsRef = useRef<Points>(null!)
  const materialRef = useRef<PointsMaterial>(null!)
  
  // Use selective state subscription from hybrid store
  ...
  const baseLayer = useHybridStore((state) => state.layers.base)

  const galaxyData = useMemo(() => {
    console.log('🌌 BaseGalaxy: Regenerating with:', { particleCount, galaxyRadius, spiralArms, spiralTightness })
  ...
  useFrame((state, delta) => {
    if (pointsRef.current && isAnimating && baseLayer.visible) {
  ...
  useEffect(() => {
    if (materialRef.current) {
      ...
      materialRef.current.opacity = baseLayer.opacity
    }
  }, [baseLayer.opacity])

  // Hide/show based on layer visibility
  useEffect(() => {
    if (pointsRef.current) {
      pointsRef.current.visible = baseLayer.visible
    }
  }, [baseLayer.visible])

  return (
    <points ref={pointsRef} visible={baseLayer.visible}>
      <bufferGeometry>
        <bufferAttribute
